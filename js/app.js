...
showNotes(); // if we refresh the page this shows old notes from local storage
    ...
    notesObj.forEach(function (element, index) {
        ...
        <div class="card-body">
            ...
            <button id="${index}" onclick="deleteNote(this.id)" class="btn btn-primary">Delete Note</button>
                ...
                `
    }); // add notes for eeach time we add
    let notesElm = document.getElementById("notes");
    if (notesObj.length != 0) {
        notesElm.innerHTML = html; // show note to display
    } else {
        notesElm.innerHTML = ` Nothing to show! Use "Add a note" section above to add notes.` // if it has no text to display, then display this
    }
}
